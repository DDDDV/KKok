...
分离结果为 44.1 kHz、双声道 Float32 WAV，可从歌曲详情或伴奏库导出。
删除分离结果保留原曲；删除歌曲同时删除对应分离结果。两种操作都保留已保存的演唱。没有评分功能。

## 录制、调整、回放与导出演唱

1. 从伴奏库或歌曲详情点击“进入演唱”，进入全屏舞台后点击麦克风开始录制。
   首次使用会请求麦克风权限；未授权不会创建录音。
2. 伴奏从开头播放，麦克风同步录制；歌词跟随伴奏时钟，界面显示麦克风音量。
...
5. “导出已保存演唱（含伴奏）”分享保存后的 WAV；“仅导出原始录音”分享未添加音效的麦克风音轨。
   “我的演唱” Tab 可搜索并打开历史演唱，再次调整、保存，或确认后删除某次演唱。
   退出时可选择保存或放弃本次调整；放弃不会影响上次保存的作品。

建议使用有线耳机。当前不监听/外放麦克风，也没有回声消除；扬声器外放的伴奏可能被
麦克风重复录入。蓝牙使用 HFP 录音通路，可能降低音质或带来可感知延迟；尚未实现设备延迟校准。

实现采用 `AVAudioPlayer` 和 `AVAudioRecorder` 的同一音频设备时钟，预定在未来 0.3 秒
共同启动，录音时使用 `.playAndRecord`。麦克风单声道 PCM 以设备采样率记录；结束后由
