        ...
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                Text("\(Int((editor.settings.vocalVolume * 100).rounded()))%")
                    .font(.subheadline.monospacedDigit()).foregroundStyle(StudioTheme.mint)
            }
            Slider(value: $editor.settings.vocalVolume, in: 0...2, step: 0.05)
                .tint(StudioTheme.mint).accessibilityLabel("我的人声音量")
                ...
                ForEach(VocalEffect.allCases) { effect in
                    Button { editor.settings.effect = effect } label: {
                        Label(effect.title, systemImage: effect.symbol)
                            ...
                            .background(editor.settings.effect == effect ? StudioTheme.mint.opacity(0.2) : Color.white.opacity(0.05),
                                        in: RoundedRectangle(cornerRadius: 12))
                            .overlay(RoundedRectangle(cornerRadius: 12)
                                .stroke(editor.settings.effect == effect ? StudioTheme.mint : .clear, lineWidth: 1))
                    }.buttonStyle(.plain)
