...
分离结果为 44.1 kHz、双声道 Float32 WAV。完成的演唱单独持久保存在本机，不受会话缓存清理影响。没有评分功能。

## 录制、回放与导出演唱

1. 分离完成后点击“开始演唱”。首次使用会请求麦克风权限；未授权不会创建录音。
...
演唱位于 Application Support 的 `SingingPerformances/<UUID>`，与导入/分离缓存分开；
