...
3. 点击“结束演唱”，或伴奏播放完毕后自动结束。保存完成后进入回放页，可拖动进度、
   暂停、继续，并按当次保存的歌词查看演唱。
4. “导出演唱（含伴奏）”分享与回放相同的 WAV；“仅导出我的录音”保留独立麦克风音轨。
   “我的演唱” Tab 可搜索并打开历史演唱，或确认后删除某次演唱。
...
现有 `AudioInputPreparer` 转换到 44.1 kHz 双声道，分块按“录音 + 0.7 × 伴奏”混音。
...
保存曲名、日期、当次歌词、混音和原始录音。草稿先持久化伴奏和元数据，保存失败或进程
结束后可重试；成功后才删除草稿伴奏与临时文件。来电、耳机插拔或进入后台会结束演唱并
