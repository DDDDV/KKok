...
1. 从伴奏库或歌曲详情点击“进入演唱”，进入全屏舞台后可先拖动进度或点选歌词，再点击麦克风开始录制。
   首次使用会请求麦克风权限；未授权不会创建录音。
2. 默认从头演唱；选择歌词后，从该句前 3 秒播放伴奏并倒计时，再从句首收录人声。
   启用评分时，首次演唱会先在本机分析原唱参考旋律并缓存；
...
保存和导出保留整首伴奏，提前结束也保留剩余的伴奏；调音试听和再次保存使用相同的整曲时间轴。

开始演唱前，可拖动下方进度条定位到当前歌词句首，也可滚动歌词并点选要唱的句子，
再点击麦克风开始。伴奏从该句前 3 秒播放，舞台显示“3、2、1”；倒计时结束后才收录人声。
如果所选句距歌曲开头不足 3 秒，先用静默补足准备时间，再从歌曲开头播放伴奏。
没有同步歌词时，进度条直接选择开唱时间；“从头开始”清除选句并恢复原有从头演唱。
录音前段补静音以保持人声在歌曲中的原始位置，倒计时内取消或中断不会生成空作品。
选句前跳过的歌词及提前结束后的段落不参与评分；录音过程中固定起唱位置。
